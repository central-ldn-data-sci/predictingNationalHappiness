...
all_from_WDI <- WDI(country="all", indicator=c(allIndicators[,1]),start=2014, end=2016)
...
for(i in 1:6){
scatterplotMatrix(WDI_Final[c(( ((i-1)*3) + 4 ):(((i-1)*3) + 6),43)])
}
dev.off()
...
scatterplotMatrix(WDI_Final[as.numeric(c(best.correlates[(((i-1)*2)+1):(2*i)],43))])
}
dev.off()
